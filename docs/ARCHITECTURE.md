...
### Data Layer
`db/schema.rs` (migrations) · `db/transactions.rs` (CRUD) · `db/price_cache.rs` (spot cache) · `db/price_history.rs` (daily history, merge) · `db/technical_snapshots.rs` (persisted technical state) · `db/technical_levels.rs` (market structure levels: support, resistance, MA, swing, range) · `db/allocations.rs` (% mode) · `db/watchlist.rs`
