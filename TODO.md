...
### Auto cash deduct/add on `portfolio transaction add`
**Source:** Skylar (May 28, live use). Added a 2oz physical gold buy (tx #21, GC=F @ \$4,500/oz) during the post-/pftui-report PCE-day add. Holding qty updated correctly (17.89 → 19.89 oz, allocation 21.84% → 24.11%) BUT cash was not debited — portfolio total inflated from \$361,577 to \$370,405. Required a manual follow-up `pftui portfolio set-cash USD 173800` (tx #22) to reconcile. The set-cash workaround REPLACES all USD transactions for that currency, losing the audit trail of the actual cash movement linked to the gold buy.
**Why:** `portfolio transaction add` treats every leg as standalone — a buy adds the asset position but never debits the paired cash leg. Result: portfolio total is silently wrong between the buy and the manual cash reconciliation, allocation percentages are wrong by the same delta, daily P&L is misattributed, and the link between the asset leg and the cash leg is lost (set-cash collapses the entire USD history into one row). Affects every transaction add — buy, sell, deposit, withdrawal — and every script/agent that calls `transaction add` without remembering to manually adjust cash after. This is the same accounting error that broker imports avoid because they record both legs of every settlement.
**Scope:** (1) Add `--cash-currency <CCY>` (default `USD`) and `--no-auto-cash` (default OFF — auto-cash on by default for `buy`/`sell` tx_types) to `pftui portfolio transaction add`. (2) When auto-cash is active and tx_type is `buy`: insert a paired cash transaction debiting `quantity × price` from the specified cash position, with notes `paired with tx #<asset_id>` and a new `paired_tx_id` column on `transactions`. (3) Symmetric for `sell`: credit cash. (4) For non-`USD` price currencies: convert at the current FX rate from `fx_cache` and record both the original-currency leg and the FX rate used in notes. (5) `transaction remove` should remove BOTH legs of a paired pair (with `--unpaired` flag to override). (6) `transaction list --paired` to show buy/sell pairs together. (7) `set-cash` continues to exist as the manual-reconciliation lever but should print a warning when it would discard paired legs. (8) AGENTS.md update documenting the new flag. (9) Migration adding `paired_tx_id INTEGER REFERENCES transactions(id)` column. (10) Tests covering: buy auto-debit, sell auto-credit, FX conversion path, pair removal, `--no-auto-cash` opt-out, set-cash warning. Files: `src/commands/transactions.rs`, `src/db/transactions.rs`, `src/cli.rs`, `src/db/schema.rs` (migration), `AGENTS.md`.
**Effort:** 4–6 hours.
...
### [Done] Fix prediction market --category filter for geopolitical/Iran contracts
**Source:** low-timeframe-analyst (Apr 7, 72/78).
