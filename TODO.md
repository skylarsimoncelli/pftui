...
- **(a) `bottom-signals`-aware alert kind.** Today `analytics alerts add --kind technical|indicator` arms individual indicators but cannot watch the *composite* N/7 (e.g. "alert when BTC monthly bottom-signals crosses ≥5/7"). Add a `cycle-bottom`/`confluence` alert kind whose check reads `cycle_bottom_signals(...).met_count` against a threshold. Layer: alert-rules (existing table — add a kind, no new table). Consumer: `analytics alerts check` + the operator accumulation alerts.
- **(b) Composite-suite backtest.** Per-criterion backtesting works via `analytics strategy` (documented), but nothing scores the *whole suite's* hit-rate at historic cycle lows — i.e. "did N/7 ≥ 5 reliably precede the low-to-low recovery?". A backtest harness that walks history, computes the suite at each bar, and measures forward returns conditioned on N/7 thresholds would make the "confluence marks lows" claim expectancy-gated rather than asserted (EPISTEMICS discipline). No new table if results are reported, not stored.
Update docs/CYCLE-SIGNALS.md's "How to backtest reliability" TODO note when these land.
