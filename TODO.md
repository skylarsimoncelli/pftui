...
### Cycle-signal backtest — broaden the verified-anchor base (research-gated, needs operator sources)
**Source:** 2026-06-25 end-to-end eval (gap #5). Verified-anchor RELIABILITY (precision/coverage vs known cycle lows) is permanently small-n: BTC/gold have only ~3 documented cycle-low anchors and cycle TOPS have zero doctrine anchors (price-structure only). The methodology is already validated by the shipped `synthetic_known_edge_is_recovered_by_expectancy` test (the engine provably recovers a known planted edge) and the forward-return expectancy path is asset-agnostic, so this is no longer blocking — but to make the verified-anchor section robust, widen the doctrine anchor set by adding more verified cycle lows per asset chapter in docs/CYCLE-THEORY.md. NEEDS OPERATOR input: which assets, and cited sources for each verified low/high date. Mostly research + docs, then extend `BTC_DOCUMENTED_4Y_LOWS` / `GOLD_DOCUMENTED_CYCLE_LOWS` and add any new asset anchor tables.

### Cycle-signal Engine checklist — confirm met-criterion color semantics (operator UX call)
**Source:** 2026-06-25 iter-9 TUI parity. In the Cycles → Engine per-criterion checklist, a *met* criterion renders a green ✓ for BOTH polarities (✓ = "signal currently satisfied"), with polarity carried by the N/7 tally color + the "Cycle-top/bottom" header. A firing cycle-TOP (distribution-risk) criterion therefore shows green. If the operator finds green-on-bearish misleading, switch met-top criteria to a warning hue (loss/amber). One-function change in `top`-checklist styling in `src/tui/views/cycles.rs`; theme slots only. Needs operator preference before changing.
