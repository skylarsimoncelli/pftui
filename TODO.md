...
- [ ] **First-run bootstrap (`pftui refresh --full`)** — On first run (or when `--full` flag passed), fetch ALL free data sources in one pass and populate the SQLite cache. Show progress bar with each source. Sources: prices (Yahoo/CoinGecko), predictions (Polymarket), news (RSS feeds), COT (CFTC Socrata), sentiment (Alternative.me F&G), calendar (TradingEconomics), BLS (employment/CPI), World Bank (GDP/debt), COMEX (gold/silver inventory), on-chain (Blockchair BTC). After first run, DB is fully seeded — subsequent TUI opens just read from cache. Files: `src/commands/refresh.rs`, all `src/data/*.rs` modules
- [ ] **Smart refresh on TUI open** — When launching `pftui` (TUI mode), check cache freshness for each source. Only fetch sources that are stale (prices >15min, news >10min, predictions >1h, COT >1 week, BLS >1 month, World Bank >30 days, COMEX >24h, calendar >24h, sentiment >1h). Show brief "Updating 3 sources..." in status bar, not blocking. Files: `src/app.rs` (init), `src/data/*.rs`
- [ ] **`pftui status --data`** — Show freshness of every cached data source: last fetch time, record count, staleness indicator (✓ fresh / ⚠ stale / ✗ empty). Files: new `src/commands/status.rs`
