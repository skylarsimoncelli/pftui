...
### Data & Display
- [ ] **Multi-currency FX conversion** — Store non-USD positions natively, convert via live FX rates (infra built in `data/fx.rs` + `db/fx_cache.rs`, needs integration with position display). Files: `models/position.rs`, `commands/summary.rs`, `views/positions.rs`
