...
| **[docs/CYCLE-SIGNALS.md](docs/CYCLE-SIGNALS.md)** | The mechanical cycle-signal suite — the deterministic N-of-7 confluence checklists for BOTH cycle bottoms (`analytics cycles bottom-signals`) and cycle tops/exhaustion (`analytics cycles top-signals`): what each of the 7 criteria measures, the JSON shape, the reliability + forward-return EXPECTANCY backtests (`... backtest --expectancy`, `--detrend`), the flexible `trigger-backtest`, the native alert conditions (`cycle_{bottom,top}_<tf>_<N>` / `_criterion_` / `_component_`), and the `analytics cycles tracked` status dashboard. Read before touching `analytics/cycle_signals.rs`, `analytics/cycle_signal_backtest.rs`, or any prose that itemizes cycle top/bottom confirmations |
| **[docs/POSITIONING-MODELS.md](docs/POSITIONING-MODELS.md)** | The portfolio strategy modeller — declarative TOML model specs (base diversification + rebalancing rules), the new daily marked-to-market **portfolio simulator** (action-algebra target solver, bounded-projection normalizer, lookahead/FX/cost contract), `analytics models` CLI, and the P0→P5 staged build. Read before touching `src/analytics/portfolio_sim/` or anything that backtests a multi-asset portfolio/allocation strategy |
