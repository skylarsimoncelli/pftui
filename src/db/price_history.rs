...
pub fn upsert_history(conn: &Connection, symbol: &str, source: &str, records: &[HistoryRecord]) -> Result<()> {
    ...
    for rec in records {
        stmt.execute(params![symbol, rec.date, rec.close.to_string(), source])?;
    }
    Ok(())
}
...
mod tests {
    ...
    fn test_upsert_and_get() {
        ...
        let records = vec![
            HistoryRecord { date: "2025-01-01".into(), close: dec!(100) },
            HistoryRecord { date: "2025-01-02".into(), close: dec!(105) },
            HistoryRecord { date: "2025-01-03".into(), close: dec!(103) },
        ];
    ...
    fn test_upsert_overwrites() {
        let conn = open_in_memory();
        let r1 = vec![HistoryRecord { date: "2025-01-01".into(), close: dec!(100) }];
        upsert_history(&conn, "AAPL", "yahoo", &r1).unwrap();

        let r2 = vec![HistoryRecord { date: "2025-01-01".into(), close: dec!(200) }];
