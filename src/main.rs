...
fn main() -> Result<()> {
    ...
    let result = match cli.command {
        ...
        Some(Command::Regime { action, limit, json }) => {
            warn_deprecated("regime", "analytics macro regime");
        ...
        Some(Command::Movers {
            ...
            warn_deprecated("movers", "analytics movers");
        ...
        Some(Command::Scan {
            ...
            warn_deprecated("scan", "analytics scan");
            commands::scan::run(
                &backend,
                &config,
                filter.as_deref(),
        ...
        Some(Command::AgentMsg {
            ...
            eprintln!(
                "Warning: `pftui agent-msg` is deprecated. Use `pftui agent message` instead."
            );
            commands::agent_msg::run(
                &backend,
                &action,
        ...
        Some(Command::Correlations {
            ...
            warn_deprecated("correlations", "analytics correlations");
            commands::correlations::run(
                &backend,
                action.as_deref(),
                value.as_deref(),
        ...
        Some(Command::Alerts { action, value, json, status }) => {
            warn_deprecated("alerts", "analytics alerts");
        ...
        Some(Command::Research {
            ...
            warn_deprecated("research", "analytics research");
            let freshness_checked = match freshness.as_deref() {
                Some(v) => Some(commands::research::validate_freshness(v)?),
                None => None,
            };
        ...
        Some(Command::Trends {
            ...
            warn_deprecated("trends", "analytics trends");
            commands::trends::run(
                &backend,
                &action,
                value.as_deref(),
