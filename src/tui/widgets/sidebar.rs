...
use crate::tui::widgets::{allocation_bars, portfolio_sparkline, portfolio_stats, regime_bar};
...
pub fn render(frame: &mut Frame, area: Rect, app: &App) {
    if is_privacy_view(app) {
        ...
        if show_regime && area.height > alloc_height + stats_height + regime_height + 10 {
            // Full layout with regime: alloc bars + sparkline + stats + regime bar
