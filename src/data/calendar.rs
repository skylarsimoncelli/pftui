...
fn classify_impact(name: &str) -> String {
    ...
    // High impact events. Match a generous set of variants so feeds that
    // emit "Non Farm Payrolls" or "Non-Farm Payrolls" still resolve to
    // "high" — without this the report's binary-catalyst loader silently
    // drops NFP and CPI day from the next-week catalyst slate.
    let high_impact = [
        "fomc",
        "fed minutes",
        "federal funds",
