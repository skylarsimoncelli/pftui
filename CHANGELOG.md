...
- How: a small reusable shim `commands::cli_json::or_json_error(command, json, result)` wrapping the analytics leaf dispatch arms in `main.rs` — no change to `main()`'s top-level handler, no `process::exit`, exit code preserved. Safe because these commands bail on their error paths before writing any stdout, so the envelope is the sole stdout content.
- Tests: 2 unit tests on the shim (ok passthrough, error propagation in both modes); full `cargo test` green; clippy clean. Verified live: bad-symbol/single-asset/same-asset failures now emit valid JSON envelopes with exit 1; success paths still emit clean JSON with no envelope leak; non-`--json` still plain-text on stderr (unchanged).
- Files: `src/commands/cli_json.rs` (new), `src/commands/mod.rs`, `src/main.rs`, `AGENTS.md`.
...
- Loud degradation: `μ ≤ 0` (an asset sitting at a cycle low) makes recovery unbounded in expectation and ruin certain — flagged `reliable:false` with the Triple-Penance figures `None` rather than a misleading finite number (per EPISTEMICS doctrine). A common, load-bearing case for a cash-heavy operator pre-inflection.
- QA-before-merge (fresh agent, independent scipy recompute — verdict CONDITIONAL PROCEED, exact parity on every field, all formula derivations checked vs the LdP source): folded its findings in. MED: the Triple-Penance MaxDD is a LOG-space drop but was displayed as an arithmetic % next to the (arithmetic) CDaR-95 — overstating BTC's drawdown by 21pts (log 0.72 vs arithmetic 0.51); now converted to arithmetic `1−e^−x` in the struct so the figures are comparable. LOW: exposed the AR(1)-corrected time-to-trough (`time_to_dd_ar1_days`) and added a positive-φ (trending) test series so the AR(1) inflation path is exercised, not just deflation.
- Tests: 8 known-answer unit tests (recovery-cliff identities; risk-of-ruin = 0.2373 hand-derived; Triple-Penance T_DD=270.5d / MaxDD=27.06% LOG / MaxTuW=1082d hand-derived; max-DD reported in arithmetic space; AR(1) inflation on a trending series; μ≤0 flagged-not-silent; thin/constant guards); full `cargo test` + `cli_help_smoke` green; clippy clean. Live: BTC max-DD@95% 51% / TuW ~6.9y / ruin 58% vs gold 20% / ~5.3y / 15% at a 25% budget.
- Files: `src/analytics/survival.rs` (new), `src/commands/survival.rs` (new), `src/analytics/mod.rs`, `src/commands/mod.rs`, `src/cli.rs`, `src/main.rs`, `AGENTS.md`.


### 2026-06-20 — feat(analytics): downside (semivariance) risk-parity for the basket allocator

- What: added a fourth `analytics basket weights --method downside-risk-parity` scheme that runs the existing ERC solver on the **zero-target (Hogan-Warren) semicovariance** `SC_ij = (1/n)·Σ min(rᵢ,0)·min(rⱼ,0)` (co-movement of LOSSES only) instead of the full covariance — so the basket is sized to equalize JOINT-CRASH risk rather than symmetric volatility. The semicovariance is symmetric/PSD (a Gram matrix of the downside-clipped returns) so the Maillard fixed point applies unchanged. A new `risk_basis` field (`variance`|`semivariance`) records what the solver + risk-contributions used; portfolio vol + diversification ratio stay full-variance so they remain comparable across all four methods.
...
- QA-before-merge (fresh agent, verdict CONDITIONAL PROCEED — parity to 1.1e-16, PSD verified, reduces to symmetric RP on symmetric data): folded its findings in — added a `note` field + a test for the degenerate zero-downside asset (one that never loses → zero semivariance → ERC correctly assigns 0%, now flagged not silent); fixed the unknown-method error to list `downside-risk-parity`; relabeled the docstring zero-target/Hogan-Warren (not Estrada, which demeans first).
- Tests: 3 new unit tests (downside-RP equalizes semivariance risk contributions AND differs from symmetric RP on a down-day-correlated pair; semicovariance symmetric/downside-only/zero-when-no-losses; never-losing asset → 0% + note); full `cargo test` + `cli_help_smoke` green; clippy clean.
...
- Why: directly serves the operator's cycle-accumulation plan — "how should I split new capital across BTC/gold/SPY?". Risk-parity surfaces that BTC's 66%/yr vol earns it only ~11% weight at equal risk budget, while the BTC↔gold near-zero correlation (inverse-vol RC 50/50) is exactly the diversification the thesis rests on. Research-scout runner-up #3, sequenced after the risk primitives existed.
- Tests: 6 known-answer unit tests (inverse-vol ∝ 1/σ, ERC equalizes risk contributions on a correlated pair, ERC=inverse-vol when uncorrelated, diversification ratio ≥1 and rises as correlation falls, small-input guards, equal-weight uniformity); full `cargo test` + `cli_help_smoke` green; clippy clean. Live: BTC/gold/SPY risk-parity → 10.8/47.5/41.7% weights, all RC=33.3%, port vol 14.2%/yr, div-ratio 1.57.
- Files: `src/analytics/basket.rs` (new), `src/commands/basket.rs` (new), `src/analytics/mod.rs`, `src/commands/mod.rs`, `src/cli.rs`, `src/main.rs`, `AGENTS.md`.
...
- Why this one: research scout (hedge-fund/insurance/actuarial + Riskfolio-Lib/empyrical) flagged drawdown-path risk as the only structural tearsheet gap; CDaR gives a coherent risk-budget denominator for a cycle accumulator that Calmar's single-point max-DD does not. Source basis: Chekhlov-Uryasev-Zabarankin (CDaR), Peter Martin (Ulcer/UPI), Keating-Shadwick (Omega).
- Tests: 6 known-answer unit tests (zero-DD curve, hand-computed CDaR/Ulcer/Omega vectors, too-few-points None, β-monotonicity); full `cargo test` green; clippy clean. Verified live: BTC risk-dashboard CDaR-95 78% / Ulcer 43% (matches its −83% max-DD history); SPY RSI backtest CDaR-95 32% ≥ CDaR-90 30% (monotone).
- Files: `src/analytics/drawdown_metrics.rs` (new), `src/analytics/mod.rs`, `src/analytics/strategy/engine.rs`, `src/commands/strategy.rs`, `src/commands/risk_dashboard.rs`, `AGENTS.md`.
...
- Also: strengthened the `fisher_high_at_top_of_range` test threshold from >1.0 to >4.0 per the Fisher-Transform QA pass (actual ≈7.6 — the old floor was too weak to catch a broken EMA/normalization).
- Tests: `cargo test` green (3 new supertrend tests: uptrend line-below-price, trend-reversal flip, param guards); clippy clean; verified end-to-end on SPY (supertrend_dir flips ~4×/yr over 33y; backtest produced 132 trades with sane stats).
- Files: `src/indicators/trend.rs`, `src/indicators/mod.rs`, `src/analytics/strategy/parser.rs`, `src/analytics/strategy/resolver.rs`, `src/indicators/momentum.rs`, `src/cli.rs`, `AGENTS.md`.
...
- Tests: 1 new indicator test (rising-to-top-of-range → strongly positive Fisher; flat series → 0); momentum tests green; full `cargo test` green; clippy clean.
- Files: `src/indicators/momentum.rs`, `src/indicators/mod.rs`, `src/analytics/strategy/{parser,resolver}.rs`, `src/cli.rs`, `AGENTS.md`, `CHANGELOG.md`.

### 2026-06-20 — fix(analytics): Hurst DFA cross-check threshold + framing (fresh-agent QA)

- What: a fractal-analysis fresh agent verified the DFA algorithm is textbook-correct (linfit, fluctuation, profile, slope all hand-checked) but found the agree/DIVERGE signal unreliable: at the `|Δ|<0.07` threshold, ~40% of genuinely same-regime series would false-flag DIVERGE from estimator sampling noise ALONE (each estimator's SE ~0.06–0.07; the diff-SE ~0.09), compounded by a ~0.03 systematic null offset (R/S centers ~0.48, DFA-1 ~0.51). Loosened the threshold to 0.13 (≈1.5× the joint sampling SE). Also: the QA showed DFA-1 does NOT remove a trend in the RETURNS input (a linear return-trend becomes quadratic in the integrated profile — DFA-1's linear detrend can't kill it, measured α=1.41), and since the command always feeds log-returns the "DFA is trend-robust, trust it over R/S" framing didn't apply — reworded the doc + verdict to position DFA honestly as an INDEPENDENT second estimator (different bias structure), not trend protection. Finally, `|Δ|` now prints to 3 decimals so the displayed magnitude can't contradict the agree/DIVERGE verdict at the boundary. Real assets unaffected (BTC/gold/SPY/us10y all agree with |Δ| 0.001–0.017).
- Tests: hurst_rs tests (8) green; full `cargo test` green; clippy clean.
- Files: `src/analytics/hurst_rs.rs`, `CHANGELOG.md`.

### 2026-06-20 — feat(analytics): risk-dashboard capstone (the risk-side analogue of positioning)

- What: the measurement suite's risk primitives were scattered across commands; new `pftui analytics risk-dashboard --asset SYM` composes them into one auditable view — annualized vol + max drawdown + current %-from-ATH; EVT tail-risk (ξ tail-fatness, 1d VaR 99/99.9%, ES99); the Hurst/DFA regime + CUSUM drift-break read; and co-crash tail-dependence vs a partner (default gold, or BTC for gold, overridable with `--vs`) — plus a one-line composite risk read that foregrounds fat tails / high vol / co-crash exposure. It's the risk-side analogue of `positioning` (which composes the direction primitives): every line is the SAME computation as its dedicated command, so the dashboard adds zero new math, just consolidation. Live: BTC shows 55%/yr vol, −83% max drawdown, ξ 0.14 (moderate fat tail) with 99.9% 1d VaR 18%, random-walk regime (R/S+DFA agree, drift turned down Jan-2026), and WEAK co-crash dependence vs gold → "Composite: high vol; co-crash risk LOW (diversification intact)"; gold shows the lower-vol, smaller-tail profile.
- Tests: full `cargo test` green (4010 — reuses already-tested pure functions); clippy clean; `cli_help_smoke` green.
- Files: `src/commands/risk_dashboard.rs` (new), `src/commands/mod.rs`, `src/cli.rs`, `src/main.rs`, `AGENTS.md`, `CHANGELOG.md`.
