...
- Why: F52 — the refresh pipeline was the system's backbone but ran all ~17 sources sequentially even though most are independent. Parallel fetching reduces total refresh time significantly. Source policies formalize the freshness thresholds and retry logic that were hardcoded throughout the function. Structured JSON output enables agent observability of refresh health. The DAG model makes dependency relationships explicit and prevents future regressions where analytics run before prices complete.
- Files: `src/commands/refresh.rs`, `src/commands/refresh_dag.rs` (new), `src/commands/mod.rs`, `src/cli.rs`, `src/main.rs`, `TODO.md`, `CHANGELOG.md`
- Tests: `cargo test` — 1418 pass (7 new: `dag_layer_ordering`, `source_status_serialize`, `refresh_result_tracks_failures`, `default_policies_cover_all_sources`, `yahoo_has_low_concurrency`, `source_result_json_serialization`, `refresh_result_json_structure`); `cargo clippy --all-targets -- -D warnings` clean
- PR: #63
