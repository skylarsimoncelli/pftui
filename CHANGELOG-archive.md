...
> Entries older than 2 weeks. See CHANGELOG.md for recent work.
...
- Why: #1 feedback request from testers — headless CLI access to portfolio value without launching the TUI. Enables integration with daily briefings, scripts, and agent workflows. Example output: `Portfolio: $250,000.00 (+3,000.00 / +1.2%)` with `Cash 40%, Commodity 35%, Crypto 25%` breakdown.
...
### 2026-03-03 — Add chart area fill with gradient background

- What: price charts now render a subtle filled-area effect below the braille line. Cells below the chart line get a dim tinted background that fades from ~15% opacity near the line to ~4% at the chart bottom, creating a polished "filled area chart" look common in financial dashboards. Applied to both full-size charts and compact mini-panel charts. The fill color derives from the same gain/loss gradient as the line itself, blended toward the surface background via `lerp_color`. New `area_fill_bg()` function computes per-cell background tint based on the chart line position relative to each row. Two constants `FILL_OPACITY_NEAR` (0.15) and `FILL_OPACITY_FAR` (0.04) control the gradient intensity range.
- Why: P2 chart visual enhancement — bare braille dots on a flat background looked sparse. The area fill adds depth and visual weight, making charts feel more like a real financial terminal. The subtle gradient avoids overwhelming the braille line while giving the chart a premium filled-area aesthetic.
...
### 2026-03-03 — Global asset search overlay (/ key)

- What: replaced the inline position-only search filter with a full-screen global asset search overlay. Pressing `/` now opens a centered popup that searches ALL 130+ assets in the symbol database (not just portfolio positions). The overlay shows each result with: symbol (bold), name, category (capitalized), cached price, and day change %. Portfolio positions are marked with ◆ (green), watchlist entries with ○ (accent). Results are fuzzy-matched and ranked (exact > prefix > substring > subsequence), capped at 20 for performance. Arrow keys navigate results, Enter selects (navigates to position in portfolio with detail popup, or selects in watchlist), Esc dismisses. Includes animated blinking cursor, scroll support for long result lists, and empty/no-match states with hints.
- Why: P0 Owner Request — the old `/` search only filtered existing portfolio positions, making it useless for asset discovery and research. The new overlay searches the full asset database, enabling users to find and explore any asset regardless of whether it's in their portfolio. This is the discovery/research entry point.
- Files: new `src/tui/views/search_overlay.rs` (SearchResult struct, `build_results()`, `render()`, 10 tests), `src/app.rs` (new overlay state fields, `handle_search_overlay_key()` method, removed dead `search_mode`/`search_query` fields and filter code, rewrote 9 old search tests as 10 new overlay tests), `src/tui/ui.rs` (render overlay), `src/tui/views/help.rs` (updated `/` description), `src/tui/views/mod.rs` (register module), `src/tui/widgets/status_bar.rs` (removed dead search mode display and filter indicator)
- Tests: 10 new overlay tests (empty/whitespace queries, symbol/name search, case insensitive, result cap, category correctness, portfolio membership detection, non-portfolio detection). 10 new key-handling tests (slash opens overlay, typing updates query, esc closes, enter closes, backspace, arrow navigation, blocks normal keys, enter selects portfolio position, typing resets selection). Total: 635 tests passing.
- TODO: Global asset search overlay (P0)

### 2026-03-03 — Promote watchlist to top-level tab [5]

- What: promoted the watchlist from a sub-tab under Positions (toggled with `w`) to a proper top-level tab accessible via `[5]` or `w` from any view. The header now shows five tabs: `[1]Pos [2]Tx [3]Mkt [4]Econ [5]Watch`. Removed the `MainTab` enum entirely — watchlist is now `ViewMode::Watchlist`, a first-class peer of Positions, Transactions, Markets, and Economy. All navigation keys (j/k/gg/G/Ctrl+d/Ctrl+u) work in the Watchlist view. The search overlay navigates directly to the Watchlist view when selecting a watched asset. The ticker tape marquee is shown on both Positions and Watchlist views.
- Why: P0 Owner Request — the watchlist was buried as a sub-tab under Positions, requiring the user to be in Positions view and press `w` to toggle. This made it undiscoverable and awkward. As a proper top-level tab, it's instantly accessible from anywhere with `5` or `w`, consistent with the other tab paradigm.
- Files: `src/app.rs` (added `Watchlist` to `ViewMode`, removed `MainTab` enum and `main_tab` field, updated key handlers for `5`/`w`, updated all 6 navigation methods, updated search overlay navigation, updated breadcrumb, rewrote 6 watchlist tab tests), `src/tui/ui.rs` (removed `MainTab` import, simplified `render_positions_layout`, added `render_watchlist_layout`), `src/tui/views/help.rs` (updated key hint to `5 / w`), `src/tui/widgets/header.rs` (added `[5]Watch` tab, extended ticker tape to Watchlist view)
- Tests: 6 new watchlist tab tests (default view, w switches, 5 switches, 1 returns, w from any view, round-trip). Total: 635 tests passing.
- TODO: Promote watchlist to top-level tab (P0)

### 2026-03-03 — Add mouse click and scroll wheel support

- What: implemented mouse support across the entire TUI. Scroll wheel navigates up/down in all five views (Positions, Transactions, Markets, Economy, Watchlist). Left click on header tab labels ([1]Pos [2]Tx [3]Mkt [4]Econ [5]Watch) switches views — tab hit zones adapt to percentage mode (where Tx is hidden) and compact layout. Left click on table rows selects that position/item. Click anywhere dismisses help overlay, search overlay, and detail popup. Mouse events were already captured by crossterm (EnableMouseCapture was set) but were being silently discarded — now they're forwarded through the event handler and processed by a new `handle_mouse` method with helper methods `handle_header_click` and `handle_content_click` for hit-testing.
- Why: P1 Owner Request — mouse support was explicitly requested ("like htop, all UI elements that look clickable should be clickable"). This is the foundation layer: scroll wheel + tab clicks + row selection cover the most common mouse interactions. Future runs can add click-to-sort on column headers, theme/privacy indicator clicks, and right-click context menus.
- Files: `src/tui/event.rs` (added `Mouse(MouseEvent)` variant, forward mouse events), `src/tui/mod.rs` (dispatch mouse events to `app.handle_mouse()`), `src/app.rs` (new `handle_mouse`, `handle_header_click`, `handle_content_click` methods; 17 new tests), `src/tui/views/help.rs` (added Mouse section to help overlay), `docs/README.md` (added mouse entries to key table), `docs/KEYBINDINGS.md` (added Mouse section)
- Tests: 17 new mouse tests (scroll up/down, clamp at bounds, overlay dismiss on click, tab switching via click, percentage mode tab adaptation, row selection, out-of-bounds click safety, markets/economy scroll, right-click ignored). Total: 652 tests passing.
- TODO: Add mouse click support (P1)

### 2026-03-03 — Restructure left pane as "PORTFOLIO OVERVIEW"

- What: restructured the left pane of the Positions view to be a comprehensive "PORTFOLIO OVERVIEW" section. Changed the section header from "POSITIONS" to "PORTFOLIO OVERVIEW". Added a new `portfolio_stats` widget below the portfolio sparkline that displays: (1) position count and category count, (2) top performer by day change % with green ▲ arrow and symbol, (3) worst performer by day change % with red ▼ arrow and symbol. The stats panel uses `compute_change_pct` from the positions view for accurate day-over-day calculation. Supports privacy mode (shows masked "••••" placeholders). The sidebar layout is now: allocation bars → portfolio sparkline → portfolio stats (3 lines). The top_movers widget is superseded but retained in the codebase for potential future use.
- Why: P1 Owner Request — the left pane lacked a cohesive identity and didn't surface key portfolio insights at a glance. The new PORTFOLIO OVERVIEW header gives the section a clear identity, and the stats panel provides instant visibility into portfolio composition and daily movers without requiring the user to scan the full positions table.
...
### 2026-03-03 — Add regime intelligence health bar (9 cross-asset signals)

- What: implemented a composite risk-on/risk-off regime intelligence system. New `src/regime/mod.rs` computes a regime score from 9 cross-asset signals: VIX level (<20 = risk-on), VIX 5D direction, 10Y yield direction, 2Y-10Y spread (normal/inverted curve), DXY direction, Gold/SPX ratio trend, BTC/SPX 20D correlation (Pearson), HY credit spread (HYG/LQD ratio), and Copper/Gold ratio. Each signal scores +1 (risk-on) or -1 (risk-off). New `src/tui/widgets/regime_bar.rs` renders a compact gauge in the sidebar: regime label (RISK-ON / LEAN RISK-ON / NEUTRAL / LEAN RISK-OFF / RISK-OFF), gradient-colored fill bar, score ratio (e.g. +7/9), and individual signal breakdown with green/red coloring. Added HYG, LQD, and Copper futures to the Markets view to ensure regime signal data coverage. Regime score recomputes on every price or history update. Only renders when ≥3 signals have data; shows "Waiting for data..." placeholder otherwise. Layout adapts gracefully — regime bar appears below portfolio stats in the sidebar only when terminal height permits.
- Why: P1 Regime Intelligence (Sentinel Proposed) — this is the first of two regime items. The health bar provides instant macro context without leaving the portfolio view. Professional-grade cross-asset signal analysis using only freely available Yahoo Finance data.
- Files: new `src/regime/mod.rs` (signal computation, Pearson correlation, 9 signal functions, 21 tests), new `src/tui/widgets/regime_bar.rs` (gauge widget, signal line renderer, 7 tests), `src/app.rs` (regime_score field, recompute_regime method, tick/init integration), `src/tui/widgets/sidebar.rs` (layout integration), `src/tui/widgets/mod.rs` (module registration), `src/tui/views/markets.rs` (added HYG, LQD, Copper; fixed count test), `src/main.rs` (regime module registration)
- Tests: 696 passing (25 new regime + widget tests, fixed pre-existing correlation test data, updated market symbol count). Zero clippy warnings on new code.
- TODO: Risk-On/Risk-Off Regime Health Bar (P1 Regime Intelligence)

### 2026-03-03 — Add regime asset suggestions with portfolio alignment

- What: added regime-aware asset suggestions that show which asset classes historically perform well or poorly in the current macro environment. New `src/regime/suggestions.rs` maps regime score to asset recommendations: risk-on favors Growth stocks, Crypto, Copper, High-yield bonds; risk-off favors Gold, Silver, Treasuries, USD, Utilities. Lean regimes show narrower sets; neutral suggests diversified/balanced. Portfolio-aware: computes what percentage of user's holdings are in regime-favored vs regime-headwind categories based on AssetCategory mapping (Equity/Crypto = risk-on strong, Forex/Cash = risk-off strong, Commodity/Fund = neutral). Displays alignment summary (e.g. "60% in regime-favored assets — well positioned" or "exposed"). New `src/tui/widgets/regime_assets.rs` renders a compact panel in the sidebar below the regime health bar showing ▲ strong assets, ▼ weak assets, and alignment summary. Supports privacy mode (masked placeholders). Dynamic height computation — only appears when terminal height permits. Truncation logic handles narrow terminals with ellipsis.
- Why: P1 Regime Intelligence (Sentinel Proposed) — second of two regime items. Completes the regime intelligence system by adding actionable context: not just "what is the regime" but "what does it mean for your holdings." Portfolio alignment closes the loop between macro signals and personal positioning.
...
### 2026-03-03 — Click allocation bars to filter positions by category

- What: added mouse click support for the allocation bars in the portfolio overview panel. Clicking any category bar immediately filters the positions table to show only positions in that category — same as pressing `f` to cycle through categories, but directly targeted. Clicking the same category again clears the filter. The active filter is visually indicated with a `▸` prefix and bold label on the filtered category's bar. Click targets are tracked during render by storing the allocation widget's absolute `Rect` area and the ordered list of rendered categories, then hit-tested in the mouse handler using row offset from the block border. Works in Positions view only (allocation bars aren't shown in other views). Stays in sync with keyboard filter cycling (`f` key) — `filter_cycle_index` is updated to match the clicked category's position.
- Why: P2 Mouse Enhancements — Click allocation bars. Natural extension of the existing mouse click interactions (tabs, theme, privacy, column headers, rows). Makes category filtering instantly discoverable — users can see the categories and click to filter without knowing the `f` keybinding.
- Files: `src/app.rs` (2 new fields: `alloc_bar_area`, `alloc_bar_categories`; `handle_alloc_bar_click` method; wired into `handle_mouse`; 7 new tests), `src/tui/widgets/allocation_bars.rs` (render signature `&App` → `&mut App`; records click targets; active filter visual indicator), `src/tui/widgets/sidebar.rs` (render signature `&App` → `&mut App`), `src/tui/ui.rs` (`render_positions_layout` signature `&App` → `&mut App`), `src/tui/views/help.rs` (new mouse help entry)
- Tests: 762 passing (7 new: click_alloc_bar_sets_category_filter, click_alloc_bar_second_category, click_alloc_bar_toggles_off_same_category, click_alloc_bar_outside_area_returns_false, click_alloc_bar_no_area_returns_false, click_alloc_bar_wrong_view_returns_false, click_alloc_bar_on_border_row_returns_false). Zero new clippy warnings.

### 2026-03-03 — Add right-click context menu on position rows

- What: added right-click context menu for position rows in the Positions view. Right-clicking any position row opens a small floating popup at the click position with four actions: View Detail (opens the detail popup), Add Transaction (opens the inline add-tx form), Delete (opens delete confirmation), and Copy Symbol (copies the ticker to clipboard via OSC 52 escape sequence). The menu renders with theme-aware styling, popup shadow, and a highlighted selection indicator (▸). Navigate with j/k or arrow keys, select with Enter, dismiss with Esc or any click. In percentage mode, transaction-related actions are excluded (only View Detail and Copy Symbol). Right-clicking also selects the clicked row before opening the menu.
- Why: P2 Mouse Enhancements — Right-click context menu. Completes the mouse interaction model: left-click selects, right-click acts. Makes all position actions discoverable without knowing keyboard shortcuts (Enter, Shift+A, Shift+X). Standard desktop UX pattern brought to the terminal.
- Files: new `src/tui/views/context_menu.rs` (render function, 4 tests), `src/tui/views/mod.rs` (registered module), `src/app.rs` (ContextMenuAction enum, ContextMenuState struct, context_menu/clipboard_osc52 fields, handle_context_menu_key, execute_context_action, right-click handler in handle_mouse, base64_encode_symbol helper, 12 new tests), `src/tui/ui.rs` (render context menu overlay), `src/tui/mod.rs` (OSC 52 clipboard write in event loop), `src/tui/views/help.rs` (new mouse help entry)
- Tests: 778 passing (16 new: 4 context_menu view tests, 3 base64 tests, 9 mouse/key interaction tests). Zero new clippy warnings.
- TODO: Right-click context menu (P2 Mouse Enhancements)

### 2026-03-03 — Add full asset detail popup from search overlay

- What: pressing Enter on a search result in the `/` overlay now opens a large centered popup (85% screen) showing everything known about the asset. Sections include: Asset Info (symbol, name, category, portfolio/watchlist status), Price (current price, 24h/7D/30D percentage changes color-coded green/red, 52-week high/low range with distance from high), Technicals (SMA(20) and SMA(50) with above/below indicators, Bollinger Band upper/lower/width, RSI(14) with overbought/oversold labels), and Portfolio Context (quantity, cost basis, value, gain/loss, allocation % — or "Watching" for watchlist items, or "Not in portfolio" for untracked assets). Esc closes the popup and returns to the search overlay (which stays open underneath). j/k and arrow keys scroll the content, g/G jump to top/bottom. Price and history data are requested on-demand for assets not already cached. Drop shadow, theme-aware styling, and scroll position indicator in the title bar.
- Why: P0 Search Overlay Enhancements — Full asset detail popup on Enter. Previously Enter only navigated to portfolio/watchlist positions, doing nothing for assets not in the portfolio. Now any searchable asset gets a comprehensive detail view with technicals, making the search overlay a powerful research tool for any asset.
- Files: new `src/tui/views/asset_detail_popup.rs` (AssetDetailState struct, render function, build_lines, SMA/Bollinger/RSI computation, 16 tests), `src/tui/views/mod.rs` (registered module), `src/app.rs` (asset_detail field, handle_asset_detail_key method, updated search Enter handler to open popup + request price data, 3 new integration tests for popup open/close/scroll), `src/tui/ui.rs` (render asset detail popup layer)
- Tests: 794 passing (16 new: 14 asset_detail_popup unit tests, 3 app integration tests — asset_detail_esc_returns, j_k_scrolls, enter_opens_detail). Zero new clippy warnings.

### 2026-03-03 — Embed braille price chart in asset detail popup

- What: added a full braille price chart section to the asset detail popup (opened via Enter in the `/` search overlay). The chart appears between the Price and Technicals sections when >= 2 history records are available. Renders an 8-row braille chart (32 dot-rows of vertical resolution) at 70 columns wide, with SMA(20) and SMA(50) overlays, area fill gradient colored by gain/loss, Y-axis min/max labels, and a stats line showing current price, gain%, H/L range, and SMA legend. The chart reuses the existing price_chart infrastructure via a new `render_braille_lines()` public function that generates `Vec<Line>` without needing a Frame — making it embeddable in any popup or text content. Background uses `surface_2` to match the popup styling. Gracefully falls back to "Insufficient chart data" message when data is unavailable.
- Why: P0 Search Overlay Enhancements — Embed braille price chart in asset detail popup. The popup previously showed price data and technicals as text only. Adding the visual chart makes it a much more powerful research tool — users can see the price trajectory, moving averages, and trend at a glance alongside the numerical data.
- Files: `src/tui/widgets/price_chart.rs` (new public `render_braille_lines()` function — 180 lines), `src/tui/views/asset_detail_popup.rs` (chart section integration, import, 3 new tests)
- Tests: 803 passing (9 new: 6 render_braille_lines tests in price_chart, 3 asset_detail_popup chart integration tests). Zero new clippy warnings.
