...
## Step 5: Fact-Check Before Publishing

🔴 **DO NOT PUBLISH UNTIL THIS STEP IS COMPLETE.**

Re-read the markdown you wrote. For every factual claim, verify it:

### 5a: Cross-check all prices
For every price, yield, index level, and percentage change in the report, verify against
a second source. Use `web_search` to spot-check at minimum:
- BTC, gold, silver, oil closing prices
- S&P 500, Nasdaq, Dow closing levels
- DXY, GBP/USD, 10Y yield
- VIX close
- Any percentage changes (recalculate: is the % consistent with the two price levels cited?)

### 5b: Cross-check all economic data
...
### 5d: Fix errors in the markdown
If you find ANY inaccuracy, fix it in the markdown file. Then regenerate the PDF:
```bash
python3 /root/pftui/agents/intelligence-report/gen-report.py \
  /root/.openclaw/workspace-finance/reports/daily-$(date +%Y-%m-%d).md \
  /root/.openclaw/workspace-finance/reports/daily-$(date +%Y-%m-%d).pdf \
...
### 5e: Log accuracy metrics
Track what you found. You will need these numbers for Step 7.
- Total data points checked
- How many were accurate
- How many needed correction
- How many came from pftui vs web search
- Which specific pftui data points were wrong (if any)

## Step 6: Commit to Repository and Update Website

Only after the fact-check passes:
...
Append a row to `/root/pftui/FEEDBACK.csv` scoring this run. Use the accuracy metrics
from Step 5.
...
- `description`: Include: (1) what % of report data came from pftui vs web search,
...
- **Maximum 25 minutes for the full run.** Ingest data, research, write, fact-check, fix errors, generate final PDF, commit, log accuracy.
