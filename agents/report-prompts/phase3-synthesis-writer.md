...
**BTC card extra requirement (cycle-clock alignment):** the BTC card must state, in one sentence inside Bull or Bear, where the current read sits vs the external cycle-analyst consensus (thesis section `cycle-frameworks`: Loukas / Camel Finance / Olson / Cowen — low clusters Oct 2026, $40k-53k zone, Loukas ~25% early-low tail) and which confirm-checklist items have printed. If the four-layer view disagrees with that external prior, say so explicitly — justified divergence is signal, silent divergence is a bug. When the HTF/macro layers surfaced a `bottom-signals` read (the mechanical N-of-7 cycle-bottom confluence, docs/CYCLE-SIGNALS.md), itemize how many of the seven criteria are firing in **plain, name-free language** ("X of 7 cycle-bottom confirmations are now firing") — public prose carries no practitioner names or indicator brand-names. The `<!--CYCLE_SIGNALS_VIZ:checklist:BTC-->` token renders the full checklist + N/7 gauge if you want it under the Bitcoin/Gold heading. When a layer cited the MEASURED reliability of that confluence (`analytics cycles bottom-signals backtest --expectancy`, or `top-signals` for a topping read), the `<!--CYCLE_BACKTEST_VIZ:expectancy:BTC?polarity=bottom-->` token renders the per-signal backtest report card (forward-return signal-vs-baseline bars + hit-rate/closeness — sign-aware bottom/top), and `<!--CYCLE_SIGNALS_VIZ:tracked:all-->` renders the live tracked-signals dashboard. Quote the LIFT-vs-baseline and confidence %, not a raw forward return, and keep the prose name-free.
