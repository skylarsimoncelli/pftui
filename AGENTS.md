...
| `pftui analytics risk-dashboard --asset SYM [--vs PARTNER] [--json]` | **Risk-side capstone** (the analogue of `positioning` for risk). Composes the measured risk primitives into one auditable view: annualized **vol** + max **drawdown** + current %-from-ATH; **EVT tail-risk** (ξ tail-fatness, 1d VaR 99/99.9%, ES99); the **Hurst/DFA regime** + CUSUM **drift-break** read; **drawdown-path risk** (CDaR-90/95 = mean depth of the worst-decile/worst-5% drawdowns — the tail of the DRAWDOWN distribution, vs max-DD's single point — plus duration-aware Ulcer Index + distribution-shape Omega ratio); **survival** (the TIME/solvency complement — risk-of-ruin vs the 25% Kelly budget + Triple-Penance max-DD/time-under-water, same as `analytics survival`); and **co-crash tail-dependence** vs a partner (default gold, or BTC for gold) — plus a one-line composite risk read that also fires on HIGH ruin (≥50%) / survivable (<15%) / no-positive-drift. Each line is the same computation as its dedicated command (EVT/hurst/regime-break/tail-dependence/risk/drawdown_metrics/survival). Live: BTC 55%/yr vol, −83% max DD, ξ 0.14, 99.9% VaR 18%, CDaR-95 78%, ruin 58% (composite: HIGH ruin) vs gold ruin 15% (survivable); co-crash vs gold WEAK (diversification holds). Needs ≥101 bars. |
| `pftui analytics basket weights --assets A,B,C [--method equal\|inverse-vol\|risk-parity\|downside-risk-parity] [--lookback N] [--json]` | **Risk-aware portfolio allocator** over a basket's common price history (read-only; no portfolio data). Four schemes: `equal` (1/N baseline), `inverse-vol` (`w_i ∝ 1/σ_i` — equalizes standalone risk, ignores correlation), `risk-parity` (ERC — equal risk CONTRIBUTION via the Maillard fixed point, using the full covariance so each asset adds the same share of portfolio variance), `downside-risk-parity` (ERC on the Estrada SEMIcovariance `min(rᵢ,0)·min(rⱼ,0)` — equalizes co-DOWNSIDE risk, sizing for joint-crash behaviour rather than symmetric vol; `risk_basis`=`semivariance`, while portfolio-vol + diversification stay full-variance for cross-method comparability). Reports each asset's weight + annualized vol + risk-contribution, the portfolio vol, and the diversification ratio (`Σwᵢσᵢ/σ_p` ≥1; higher = more benefit captured). `--lookback` windows the common axis (0 = all); needs ≥21 common dates. Tickers with `^`/`=`/`-` use their alias. Live: BTC/gold/SPY risk-parity → BTC 10.8% / gold 47.5% / SPY 41.7% (all RC=33.3%), port vol 14.2%/yr, div-ratio 1.57; downside-RP → BTC 11.7% (its co-downside is milder than its symmetric vol); BTC/gold inverse-vol RC 50/50 confirms their near-zero correlation. Directly serves capital-split decisions across the held basket. |
| `pftui analytics survival --asset SYM [--budget PCT] [--confidence 0.95] [--lookback N] [--json]` | **Drawdown survival & recovery** — the TIME + solvency axis that complements the depth-only EVT/CDaR views (Bailey & López de Prado, Triple Penance). Read-only over `price_history`. Reports: the **recovery cliff** `D/(1−D)` on the measured CDaR-95 (a 50% loss needs +100%, 80% needs +400%); **Triple Penance** expected max-drawdown at confidence α, time-to-trough, and total time-under-water (≈4·T_DD), each shown both i.i.d. AND with an **AR(1) serial-correlation correction** (`σ²·(1+φ)/(1−φ)` — trending cycles understate underwater time on the i.i.d. view); and **risk of ruin** `exp(−2μ·b/σ²)` (P of ever breaching the `--budget` drawdown, default 25%). `μ ≤ 0` (an asset at a cycle low) makes recovery unbounded / ruin certain → flagged `reliable:false` with the model figures `None`, not a misleading number. Live: BTC max-DD@95% 51%, time-under-water ~6.9y, ruin **58%** vs a 25% budget; gold max-DD 20%, TuW ~5.3y, ruin **15%** — the survival contrast a cycle accumulator sizes on. Needs ≥31 closes. |
...
| `pftui analytics cycles bottom-signals --asset BTC --timeframe monthly --json` | Mechanical N-of-7 cycle-low signal suite. Exposes ordered `criteria[]`, focused `core_watch[]`, per-component previous/current comparison values, signed distance-to-trigger where computable, and optional pi-cycle bottom bonus not counted in the 7. Backtest with `pftui analytics cycles bottom-signals backtest --asset BTC --json`; point-in-time/no-lookahead against verified cycle lows. Alerts: `cycle_bottom_<tf>_<N>`, `cycle_criterion_<tf>_<key>`, `cycle_component_<tf>_<key>`. |
| `pftui analytics cycles top-signals --asset BTC --timeframe monthly --json` | Mechanical N-of-7 cycle-high / exhaustion signal suite. Same JSON shape as `bottom-signals`, inverted toward top-risk criteria, with optional pi-cycle top bonus not counted in the 7. Backtest with `pftui analytics cycles top-signals backtest --asset BTC --json`; point-in-time/no-lookahead against completed native highs derived as the max close between verified lows, excluding the unfinished current cycle. Alerts: `cycle_top_<tf>_<N>`, `cycle_top_criterion_<tf>_<key>`, `cycle_top_component_<tf>_<key>`. |
| `pftui analytics cycles bottom-signals trigger-backtest --asset BTC --trigger rsi_ma_cross_above_rsi --horizons 7d,30d,365d --json` | Native cycle-low event study for arbitrary criterion/component keys and combinations. Emits every historical firing, nearest verified low, signed day distance, signed price distance from the low, forward returns at custom horizons, and aggregate good-rate/mean/median returns. Combine keys with comma-separated/repeated `--trigger`; `--mode all` (default) requires every key, `--mode any` requires one. |
| `pftui analytics cycles top-signals trigger-backtest --asset BTC --trigger rsi_ma_cross_below_rsi --horizons 7d,30d,365d --json` | Symmetric native cycle-high / exhaustion event study. Anchors are completed native highs derived from verified low-to-low cycles; `good` forward returns are negative after the top signal. Works for BTC and gold/silver where native cycle anchors exist. |
...
| `pftui analytics models list \| show <name> --json` | The **positioning modeller** (engine: `analytics/portfolio_sim/`, full design: docs/POSITIONING-MODELS.md). A *model* is a declarative TOML spec in `models/*.toml` = universe (symbols→class) + base_policy (per-class target weights with floor/ceiling drift bands) + `[[rules]]` (when/then) + `[params]`. `list` enumerates specs; `show` parses+validates and prints the resolved model (classes, targets, cadence, rule count). Validation rejects phantom target classes, infeasible bands, unknown enums |
| `pftui analytics models backtest <name> [--from --to] [--json]` | Daily marked-to-market portfolio backtest over real price_history: own union calendar, Decimal cash/position/fee ledger, FX converted once at mark/fill date, next-close fills (lookahead-safe), bounded-projection target solver, phased action algebra, `no_average_down` enforced vs the sim's own avg-cost. Reports daily-curve metrics (CAGR/Sharpe/Sortino/Calmar/maxDD/CDaR/Ulcer/vol/turnover/time-in-cash/costs) + **three benchmarks** (static base-policy, rebalanced base-policy [= rule-alpha isolator], equal-weight). Signal rules fire via lookahead-safe accessors `cycle_bottom_met`/`cycle_top_met`/`regime_score`/`stage_proxy`/`cyber_dot_up`/`cyberline_lost` with completed-bucket trimming |
| `pftui analytics models compare <a> <b> … [--from --to] [--json]` | Run 2+ models over the SAME window + cost model; ranked table (CAGR/Sharpe/Sortino/MaxDD/Calmar/Vol/Cash/Turnover/Costs/nReb), best-per-metric marked, each with its three benchmarks, + a Calmar verdict naming the best risk-adjusted model |
| `pftui analytics models optimize <name> --param NAME=min:max:step [--folds N] [--objective calmar\|sortino\|sharpe\|cagr] [--slices S] [--no-ledger] [--json]` | **Walk-forward** optimization of a model's declared numeric `[params]` ONLY (topology frozen). ~4y-warmup-aware rolling train/test folds; pick each fold's best on TRAIN, score OUT-OF-SAMPLE; net-returns-first; min-activity gate. Overfit controls: **PBO via CSCV** (primary) + **Deflated Sharpe Ratio**; an **18-month lockbox** never touched by folds; a persistent **optimization ledger** counting cumulative trials per topology-hash (meta-overfitting guardrail). Verdict ∈ insufficient-data/overfit-likely/fragile/robust (robust requires PBO≤0.25 AND DSR≥0.95). Reports "best observed OOS config under this frozen search space" — never "optimal". Refuses k_params>6 or N_configs>2000 |
| `pftui analytics models optimize-history [--json]` | Optimization-ledger audit: cumulative trials per topology family + recent runs. Repeated re-runs of the same frozen search space are silent multiple testing that the per-run PBO/DSR cannot see — this surfaces them |
...
2. **Use `--json` for programmatic access.** Every command supports it. Parse structured output instead of scraping text. On the `analytics` risk/measurement commands (`survival`, `risk-dashboard`, `tail-risk`, `tail-dependence`, `hurst`, `regime-break`, `avwap`, `basket weights`) AND the `analytics strategy` commands (`backtest`, `segment`, `compare`, `explain`, `sweep`, `walkforward`), a FAILURE under `--json` now emits a machine-readable envelope on STDOUT — `{"error": {"command": "...", "message": "..."}}` — with exit code 1 (the human-readable line still goes to stderr). So a `--json` consumer always gets parseable stdout, never an empty buffer.
